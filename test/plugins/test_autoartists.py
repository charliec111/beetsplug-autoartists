...
class AutoArtistsPluginTest(BeetsTestCase):
    ...
    def _setup_config(
        self, single_artists=["Earth, Wind & Fire", "A and B"]
    ):
    ...
    def test_whitelist(self):
        ...
        assert len(self.plugin.get_artists("Jim Croce and Another artist", "a song title")) == 2 
        assert len(self.plugin.get_artists("Jim Croce & Another artist", "a song title")) == 2
        assert len(self.plugin.get_artists(artist_whitelist[0], "September")) == 1 
        assert len(self.plugin.get_artists(artist_whitelist[0], "September")) == 1 
        assert len(self.plugin.get_artists("An Artist", f"September (feat. {artist_whitelist[0]})")) == 2 
        assert len(self.plugin.get_artists("An Artist", f"September (featuring {artist_whitelist[0]})")) == 2 
        assert len(self.plugin.get_artists("An Artist", f"September (with {artist_whitelist[0]})")) == 2 

        artist_whitelist = ["Jim with Bob", "Earth, Wind & Fire"]
        self._setup_config(single_artists=artist_whitelist)
        assert len(self.plugin.get_artists(artist_whitelist[0], "September")) == 1 

        artist_whitelist = ["AC/DC", "Daryl Hall & John Oates", "Earth, Wind & Fire", "Simon & Garfunkel", "Prince and The Revolution", "Florence + the Machine", "A with B"]
        self._setup_config(single_artists=artist_whitelist)
        for i in artist_whitelist:
            ...
            try:
                ...
                raise e
            test_result = self.plugin.get_artists(f"{i} feat. Another Artist", "Song Title")
            try:
            ...
            for separator in [", ", " & ", " + ", "/"]:
                for test_result in [
                    self.plugin.get_artists(f"B", f"Song Title (feat. {i}{separator}OtherArtist)"),
                    self.plugin.get_artists(f"B", f"Song Title (featuring {i}{separator}OtherArtist)"),
                    self.plugin.get_artists(f"B", f"Song Title (with {i}{separator}OtherArtist)"),
                ]:
                    try:
                        assert test_result in [ ["B", i, "OtherArtist"],["B", "OtherArtist", i] ]
                    except AssertionError as e:
                        print(f"Failed on test result: {test_result}")
                        raise e
            ...
            for separator in [", ", " & ", " + "]:
                for test_result in [
                    self.plugin.get_artists(f"B", f"Song Title (feat. {i}, AnArtist{separator}OtherArtist)"),
                    self.plugin.get_artists(f"B", f"Song Title (featuring {i}, AnArtist{separator}OtherArtist)"),
                    self.plugin.get_artists(f"B", f"Song Title (with {i}, AnArtist{separator}OtherArtist)"),
                ]:
                    try:
                        assert sorted(test_result) == sorted(["B",i, "AnArtist","OtherArtist"])
                    except AssertionError as e:
    ...
    def test_separating_artist(self):
        ...
        test_result = self.plugin.get_artists(artist="Beyoncé feat. JAY-Z & Kanye West", title="Drunk in Love (remix)")
        ...
        for test in [ 
            ...
            test_result = self.plugin.get_artists(test[0], test[1])
            try:
                assert test_result == ['Jim Croce', {artist_whitelist[0]}] or [{artist_whitelist[0]}, 'Jim Croce']
            except AssertionError as e:
                print(f"Failed on test result: {test_result}")
                raise e
        ...
        for test_result in [
            self.plugin.get_artists(f"Kate Bush", "Running Up That Hill (A Deal with God)"),
        ]:
            try:
        ...
        for test_result in [
            self.plugin.get_artists(f"Taylor Swift", f"Breathe (feat. Colbie Caillat) (Taylor’s version)"),
        ]:
            try:
        ...
        for test_result in [
            ...
            try:
                ...
                raise e
        try:
            assert len(self.plugin.get_artists("Taylor Swift", "Safe & Sound (feat. Joy Williams and John Paul White) (Taylor’s Version)", artists=['Taylor Swift', 'Joy Williams', ' John Paul White'])) == 3 
            assert len(self.plugin.get_artists("Taylor Swift", "Safe & Sound (feat. Joy Williams and John Paul White) (Taylor’s Version)", artists=['Taylor Swift', 'Joy Williams', 'John Paul White', ' John Paul White'])) == 3 
        except AssertionError as e:
            raise e
        for test_result in [
            self.plugin.get_artists(f"ZArtist A", f"Song Title (With Artist B)"),
            self.plugin.get_artists(f"ZArtist A & Artist B", f"Song Title (With Artist B)"),
            self.plugin.get_artists(f"ZArtist A Feat. Artist B", f"Song Title (With Artist B)"),
        ]:
            try:
                ...
                raise e
        for test_result in [
            self.plugin.get_artists(f"ZArtist A", f"Song Title (With Artist B and artist c)"),
            self.plugin.get_artists(f"ZArtist A & Artist B, artist c", f"Song Title (With Artist B)"),
            self.plugin.get_artists(f"ZArtist A Feat. Artist B & artist c", f"Song Title (With Artist B)"),
        ]:
            try:
